...
/**
 * Self-relative persistent pointer. Stores both the data offset AND the offset to its allocator_space_t.
 * Uses its own address (`this`) to calculate the current space address. This is fully persistent-safe and requires
 * no external setup when used properly (make sure its memory is properly initialized).
 *
...
public:
    ...
    PersistentPtr(allocator_space_t* space) noexcept : PersistentPtr(nullptr, space) {}
    ...
    persistent_offset_t get_data_offset() const noexcept {
        return data_offset_;
    }

    // Get the raw space offset (for debugging)
    int64_t get_space_offset() const noexcept {
        return space_offset_;
    }
    ...
    T& operator[](difference_type index) const noexcept {
        return get()[index];
    }
    ...
    difference_type operator-(const PersistentPtr& other) const noexcept {
        return static_cast<difference_type>(data_offset_ - other.data_offset_) / sizeof(T);
    }
    ...
    friend T* to_address(const PersistentPtr& p) noexcept {
        return p.get();
    }

    template<typename U>
    friend class PersistentPtr;

private:
    /*
     * Implementation note: 128bits of pointer seems a bit unexpected, but do we realistically want to pack it?
     */
    ...
    // This is signed because space could be before or after this pointer (if the PersistentPtr is placed outside of its allocator_space)
    int64_t space_offset_;
};

// Helper: Create a PersistentPtr from a raw pointer
template<typename T>
PersistentPtr<T> make_persistent_ptr(T* ptr, allocator_space_t* space) noexcept {
