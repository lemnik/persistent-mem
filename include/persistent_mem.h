 ...
 * The `origin` is set to `&allocator_space` when the file is first created,
 * allowing the file to be mapped again later into a different address and for
...
/**
 * Roots are special allocations that can be rediscovered when loading
 * persistent memory from a previous process. See [persistent_malloc_root] and
...
/**
 * Allocate a "root" for the given allocator. Roots are special regions tagged
 * with a "root class" value that can indicate something about what is stored
 * there. They can be iterated over directly from the `allocator_space` but
 * *cannot* be freed or resized. Most regions will only ever have a single root
 * as a way to "re-enter" the structures stored after the allocator has been
 ...
 * @return a pointer to at least [size] bytes of persistent memory, or NULL if
 ...
 * @return a pointer to the selected root or NULL if it cannot be found in
 * `space`.
 */
...
/**
 * Returns the correct "current" value of [ptr] for [space] where `ptr` is a
 * pointer stored within the given `space`, but where `space` potentially had a
 * different address when `ptr` was established. That is: `space` might
 * represent a memory snapshot from another process or from a previous run,
 * `ptr` was once a valid pointer but is now likely invalid. This function
 * either corrects the address, or returns `NULL` if the pointer address is
 * outside of the `space` space.
 *
 * @param space
 * @param ptr
 * @return
